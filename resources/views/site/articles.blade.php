...
@section('title', 'Articles · Cantores Hermanos')
    ...
    <div class="rounded-3xl border border-[var(--color-border)] bg-[var(--color-surface)] p-6 shadow-sm sm:p-10" data-live-listing>
        <div class="flex flex-col gap-6 md:flex-row md:items-start md:justify-between">
            <div class="max-w-3xl md:pt-1">
                <h1 class="text-3xl font-semibold tracking-tight text-slate-900 sm:text-4xl">Articles</h1>
                <p class="mt-4 text-base text-slate-700">
                    Insights, news, and stories from the Cantores Hermanos choir community.
                </p>
            </div>

            <div class="w-full max-w-md">
                <form method="GET" action="{{ route('site.articles') }}" class="w-full" data-live-form>
                        ...
                        <div class="grid grid-cols-2 gap-2 sm:grid-cols-1">
                            ...
                            <select id="per_page" name="per_page" class="min-h-11 w-full rounded-xl border border-[var(--color-border)] bg-white px-3 py-3 text-sm font-semibold text-slate-900 shadow-sm focus:border-[var(--color-primary)] focus:ring-0">
                                @foreach (($perPageOptions ?? [10, 20, 50]) as $opt)
                                    <option value="{{ $opt }}" @selected((int) ($perPage ?? 10) === (int) $opt)>{{ $opt }}/page</option>
                                @endforeach
                            </select>
        ...
        <div class="mt-8 hidden items-center gap-3 text-sm text-slate-700" data-live-loading aria-live="polite">
        ...
        <div class="mt-10" data-live-results>
            @include('site.partials.articles-results', ['articles' => $articles])
        </div>
    </div>
@endsection
