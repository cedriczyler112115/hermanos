...
@section('title', 'Add Performance · Admin')

@section('content')
    <div class="flex items-end justify-between gap-4">
        <div>
            <h1 class="text-2xl font-semibold text-slate-900">Add performance</h1>
            <p class="mt-1 text-sm text-slate-600">Create a new performance entry.</p>
        </div>
    ...
    <div class="mt-6 rounded-2xl border border-[var(--color-border)] bg-[var(--color-surface)] p-6 shadow-sm">
        <form method="POST" action="{{ route('admin.performances.store') }}" class="space-y-6" data-loading-message="Saving...">
            @csrf
